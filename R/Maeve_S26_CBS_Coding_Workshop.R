...
# Maeve Mitchell
# 5/18/2026
mtcars #practice dataset built into R
...
logicals <- c(TRUE, FALSE, TRUE) #booleans/logicals

nums[1]       		# 5 — first element (R is not 0-index)
logicals[3]      	# TRUE — last element
words[c(1,3)]  		# 5, 7 — multiple elements
nums[2:4]       	# 6, 7, 8 — a range (inclusive)
...
df$age        # 25, 30 - access age column, all rows
df[1, ]       # Alice 25 TRUE -  access row 1, all columns
df[2,2] 	    # 30 - access row 2 column 2
...
library(ggplot2)
p1<-ggplot(mtcars, aes(y=hp, x=mpg))+geom_point()
p1